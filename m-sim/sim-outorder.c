...
unsigned int cores_at_init_time = 0;
int max_contexts_per_core = 0;
std::vector<core_t> cores;

int reg_counter[MAX_CONTEXTS] = {0};
int reg_counter_size = 0;
//haque edit: global runtime-configurable per-context rename limit (-1 = use per-core)
int rename_reg_limit = 29; /* default: -1 means use per-core rename_limit */
/*
 ...
 * pipeline operations.
 */
//haque edit: add defensive underflow check to catch inconsistent frees
void reg_counter_decref_if_positive(int tid)
{
	if (tid < 0 || tid >= num_contexts)
	{
		fprintf(stderr, "reg_counter_decref_if_positive: invalid tid=%d (num_contexts=%d)\n", tid, num_contexts);
		abort();
	}

	if (reg_counter[tid] <= 0)
	{
		// Detailed diagnostic to help trace where the underflow happened
		fprintf(stderr, "reg_counter underflow detected: tid=%d reg_counter=%d\n", tid, reg_counter[tid]);
		// Print a brief stack hint if available (POSIX backtrace), but avoid adding a new dependency.
...
void sim_reg_options(opt_odb_t *odb)
{
	...
	/* global runtime-configurable per-context rename limit; if set >=0, overrides per-core setting */
	/* haque edit: use distinct option name to avoid collision with per-core registration (-rename:global_limit) */
	opt_reg_int(odb, "-rename:global_limit","",
		"global per-context limit for outstanding renamed (allocated) physical registers (-1 to use per-core)",
		&rename_reg_limit, /* default */29,
		/* print */TRUE, /* format */NULL); /*haque edit */

	/* NOTE: per-core options will register per-core rename limit below */
	...
	for(unsigned int i=0;i<cores_at_init_time;i++)
	{
		...
		opt_reg_int(odb, "-rename_dispatch_delay",offset,
			...
			//haque edit: per-core per-context rename limit registration
			/* per-core per-context rename limit */
			opt_reg_int(odb, "-rename:limit",offset,
				"per-context limit for outstanding renamed (allocated) physical registers",
				&cores[i].rename_limit, /* default */32,
				/* print */TRUE, /* format */NULL);
...
void commit(unsigned int core_num)
{
	...
	while(committed < cores[core_num].commit_width)
	{
		...
		if(contexts[context_id].ROB[contexts[context_id].ROB_head].physreg >= 0){
			...
			/*haque edit*/
			reg_counter_decref_if_positive(context_id);
...
void register_rename(unsigned int core_num)
{
	...
	while(n_renamed < cores[core_num].decode_width)
	{
		...
		//make sure we have a free physical register for the destination haque edit
		if(my_regs.dest != REG_NONE)
		{
			{
				// enforce per-thread limit on outstanding renamed (allocated) phys regs
				//haque edit: prefer global rename_reg_limit if set (>=0), otherwise use per-core setting
				int effective_rename_limit = (rename_reg_limit >= 0) ? rename_reg_limit : cores[core_num].rename_limit; //haque edit
...
//								std::cerr << "Context(" << i << ") has reached its entry point" << std::endl;
	...
	#ifdef MD_NUM_IREGS
		int _arch_regs_for_snapshot = MD_NUM_IREGS;
	#else
		int _arch_regs_for_snapshot = 32;
	#endif

	static std::vector<std::vector<int> > initial_rename;
	initial_rename.resize(num_contexts);
	for (int t = 0; t < num_contexts; ++t) {
	    initial_rename[t].resize(_arch_regs_for_snapshot);
	    for (int r = 0; r < _arch_regs_for_snapshot; ++r) {
		initial_rename[t][r] = contexts[t].rename_table[r];
	    }
	}
	//haque edit
	for (int t = 0; t < num_contexts; ++t) {
	...
	for(;;)
	{
		...
		//go to next cycle haque edit
		sim_cycle++;
		
		if (sim_cycle == 1) {
		    int arch_regs = _arch_regs_for_snapshot;
		    fprintf(stderr, "=== Cycle 0 ===\n", (unsigned long long)sim_cycle);
		    for (int t = 0; t < num_contexts; ++t) 
		    fprintf(stderr, "Thread %d used %d registers, ", t, reg_counter[t]);
			fprintf(stderr, "\n ==========================\n");
		}
